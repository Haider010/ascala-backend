...
def find_connection_for_context(context: dict) -> dict | None:
    ...
    try:
        ...
        return {"id": row[0], "companyId": row[1], "locationId": row[2]}
    finally:
        if cursor:
            cursor.close()
        if conn:
...
async def create_ghl_session(request: Request):
    ...
    connection = find_connection_for_context(context)
    if not connection:
    ...
    return {
        ...
        "storageScope": ".".join(storage_scope_parts),
    }
